...
@section('sketch')
    ...
    <form action="/admin/editPages/kontakt/{{ $kon->id }}" method="post">
        @csrf

        @method('PATCH')

        <label for="przez">Zmień górny napis:</label>
        <textarea style="height: 30px; width: 400px;" name="gora" id="gora">{{ $kon->gora }}</textarea>
        @error('gora')
        <small class="text-danger">{{ $message }}</small>
        @enderror

        <label for="przez">Zmień pierwszą informację:</label>
        <textarea style="height: 160px; width: 700px;" name="text1" id="text1"></textarea>
        @error('text1')
        <small class="text-danger">{{ $message }}</small>
        @enderror

        <label for="przez">Zmień drugą informację:</label>
    ...
    <div hidden id="token1">{{ $kon->text1 }}</div>
    <div hidden id="token2">{{ $kon->text2 }}</div>
